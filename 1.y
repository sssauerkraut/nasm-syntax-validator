...
loop_instr:
    loop_op imm_op
  | loop_op imm_op COMMA reg16_op {
        if (strcasecmp($<str>4, "cx") != 0)
            yyerror("second operand must be CX");
    }
  | loop_op imm_op COMMA reg32_op {
        if (strcasecmp($<str>4, "ecx") != 0)
            yyerror("second operand must be ECX");
    }
  ;
fpu_mem_instr_op:
    FBLD | FBSTP
