...
/* Константы */
imm8_op:   NUMBER { if($1 < -128 || $1 > 255) yyerror("imm8 out of range"); 
                    printf("    imm8: %d", $1); } ;

imm16_op:  NUMBER { if($1 < -32768 || $1 > 65535) yyerror("imm16 out of range"); 
                    printf("    imm16: %d", $1); } ;

imm32_op:  NUMBER { printf("    imm32: %d", $1); } ;

sbyte8_op: SBYTE { printf("    sbyte: %d", $1); }
          | NUMBER { if($1 > 127 || $1 < -128) yyerror("sbyte out of range"); 
                     printf("    sbyte: %d", $1); }
          ;
...
int main(void) {
    ...
    printf("Enter assembly code (Ctrl+D to finish):\n");
    yyparse();
    printf("\n=== Validation complete ===\n");
    return 0;
}